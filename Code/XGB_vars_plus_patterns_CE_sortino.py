...
if __name__ == "__main__":
    data_df = pd.read_csv('../Data/SPY_all_vars_plus_patterns.csv', encoding='utf-8', sep = ',', low_memory=False, header = 0, skiprows = 0, skipinitialspace=True, parse_dates = ['date'])
    data_df.set_index('date', inplace = True)
    # display( data_df.head() )

    use_data = 'SPY_all_vars_plus_patterns'

    use_model = 'XGB'


    use_custom_loss = True # whether or not to use a custom loss function to train a LGBM model

    # Define objective/loss functions for LGBM
    objective = closs.update_As1_lgbm
    loss_fn_name = 'As1'
    ...
    if use_model == 'LGBM':
        if use_custom_loss:
            ...
            loss_fn_name = 'CE'
        n_trials = 60 # set the number of trials for Optuna
    ...
    # scoring_fns = ['Calmar_ratio_fixed_trans_cost', 'Calmar_ratio_variable_trans_cost', 'Sharpe_ratio_variable_trans_cost', 'Sortino_ratio_fixed_trans_cost', 'Sortino_ratio_variable_trans_cost', 
    #                             'CECPP_fixed_trans_cost', 'CECPP_variable_trans_cost']   

    scoring_fns = ['Sortino_ratio_fixed_trans_cost', 'Sortino_ratio_variable_trans_cost']
    ...
    for scoring_fn in scoring_fns:
        ...
        forecasts_df, scores_df, opt_params_df, importances_df, SHAP_df = forecast.rolling_forecast(data_df,
                                                                                                                                                                ...
                                                                                                                                                                n_jobs = 70 # the number of workers for multiprocessing
                                                                                                                                                            )
